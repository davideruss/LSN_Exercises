...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    for (int i=0; i<_npart; i++){
      ...
      /*
      //------------------- Parte temporanea ----------------------------
      int a= static_cast<int> (_rnd.Rannyu(0,3));
      double val= sqrt(3*_temp);
      if(_rnd.Rannyu()<0.5) val*= -1.;
      vx(i)=0;
      vy(i)=0;
      vz(i)=0;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8* M_PI* _rho*(pow(_r_cut, -9)/9 - pow(_r_cut, -3)/3); //EXERCISE 7
        ...
        index_property++;
        _ptail = 32*M_PI*_rho *(pow(_r_cut, -9)/9 - pow(_r_cut, -3)/6); // TO BE FIXED IN EXERCISE 7
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    ...
    for (int i=0; i<_npart; i++){
      ...
      double v_mod= sqrt(dot( _particle(i).getvelocity() , _particle(i).getvelocity() ));
      
      int bin= static_cast<int>(v_mod/_bin_size_v); 
      
      if(bin<_n_bins_v) _measurement(_index_pofv+bin)++;
  ...
  if (_measure_pressure){ 
    ...
    else if(_sim_type ==1) _measurement[_index_pressure] = _rho * _temp + (_ptail*_npart + 48.0*virial/3.0)/_volume;
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){ 
...
// TO BE FIXED IN EXERCISE 6
  ...
  //La mia idea è: mi calcolo qua il termine E^2 e poi quando calcolo le averages lo sistemo

  if (_measure_cv and _measure_tenergy) _measurement(_index_cv) = pow(tenergy_temp,2);
// TO BE FIXED IN EXERCISE 6

  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_magnet and _measure_chi){ 
    _measurement[_index_chi]=_beta*pow(magnetization,2)*_npart;
  }
...
void System :: averages(int blk){
  ...
  if(_measure_cv){
    _average(_index_cv)-= pow(_average(_index_tenergy), 2);
    _average(_index_cv)*= pow(_beta, 2)*_npart;
  }
  ...
  if(_measure_gofr){
    ...
    for(int i =0; i<_n_bins; i++){
      ...
      if (blk== _nblocks){  
        ...
        double DV= 4*M_PI*(pow((i+1)*_bin_size, 3)- pow(i*_bin_size, 3))/3;
        coutf << setw(12) << i*_bin_size
              << setw(12) << sum_average/(_rho * double(_npart) *DV)
  ...
  if(_measure_pofv){
    ...
    for(int i =0; i<_n_bins_v; i++){
      ...
      int passi[]= {3, _nblocks/2, _nblocks*3/4, _nblocks};
      if (blk== passi[0] || blk== passi[1] || blk== passi[2] || blk== passi[3]){  
        coutf << setw(12) << i*_bin_size_v
              << setw(12) << sum_average/(double(blk) * double(_npart) * _bin_size_v)
              << setw(12) << this->error(sum_average, sum_ave2, blk)/double(_npart) << endl;
