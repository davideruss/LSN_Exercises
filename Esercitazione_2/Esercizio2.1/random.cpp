...
double Random :: distribuzione1() {
   // This function generates a random number from a p(x) = 2(1-x) distribution with given mean and sigma
   double s=Rannyu();
   double x=1-sqrt(1-s);
   return x;
}
...
int Random :: randint(int valore_min, int valore_max){

   int N = valore_max +1 - valore_min;
   double s = Rannyu();
   
   return static_cast<int>(floor(s*N+valore_min));
}

int Random :: choice(int valore_1, int valore_2){
   //Estrae tra 2 valori
   double s= Rannyu();
   if(s<0.5){
      return valore_1;
